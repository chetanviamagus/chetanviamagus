import { OPENAI_API_HOST } from "util/app/const";
import { cleanSourceText } from "util/server/google";

import { Message } from "util/types/chat";
import { GoogleBody, GoogleSource } from "util/types/google";
  ...
  try {
    ...
    const googleRes = await fetch(
      `https://customsearch.googleapis.com/customsearch/v1?key=${
        googleAPIKey ? googleAPIKey : getEnvVariableValue("VITE_GOOGLE_API_KEY")
      }&cx=${
        googleCSEId ? googleCSEId : getEnvVariableValue("VITE_GOOGLE_CSE_ID")
      }&q=${query}&num=5`
    );
