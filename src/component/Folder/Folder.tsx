    ...
    <>
      ...
      >
        {isRenaming ? (
          ...
          <button
            className={`flex w-full cursor-pointer items-center justify-between gap-3 rounded-lg p-3 text-sm transition-colors duration-200 hover:bg-black/30`}
            onClick={() => {
              setIsOpen(!isOpen);
            }}
          ...
          >
            ...
            {isOpen ? <ChevronUpIcon className="w-6" /> : <ChevronDownIcon className="w-6" />}
          </button>
        )}
        ...
        {!isDeleting && !isRenaming && isHovering && (
          <div className="absolute right-1 z-10 flex text-gray-300">
            <SidebarActionButton
              handleClick={(e) => {
                e.stopPropagation();
