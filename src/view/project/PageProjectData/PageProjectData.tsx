...
import { LOCAL_STORAGE, TOAST_VARIANT } from "util/Constant";
...
import { linkAuthRoute, linkProjectBase, linkProjectChat, linkProjectDatasource } from "routes";
...
const PageProjectData: React.FC<ConnectDataProps> = (props: ConnectDataProps) => {
  ...
  const handleEditNameSave = (name: string) => {
    const newData = {
      ...currentProject,
      name: name,
      projectInfo: { ...currentProject.projectInfo, projectName: name },
    };

    dispatch(setSelectedProject(newData));

    const projects = localStorage.getItem("projects");

    const _projects = JSON.parse(projects ?? "");
    const newList = _projects?.map((project: any) => {
    ...
    setLocalStorage("projects", JSON.stringify(newList));
  };
  ...
  const handleEditDescriptionSave = (description: string) => {
    const newData = {
      ...
      projectInfo: { ...currentProject.projectInfo, projectDescription: description },
    };
  ...
  return (
    <div className="flex mx-auto w-full md:max-w-rightContent p-3 flex-col">
      <div className="mb-6 flex flex-col items-center gap-3 px-3 pt-3 sm:flex-row sm:justify-between">
        <Text label={"Application Sources"} className="text-2xl font-semibold text-dark-text-main" />
        ...
        <SimpleDropDown
          ...
          onChange={(e: any) => {
            ...
            navigate(
              linkAuthRoute + `${linkProjectBase}/${e.value?.id ?? "uid"}${linkProjectDatasource}`
            );
          }}
          dataKey="id"
          variant="secondary"
      ...
      <div
        ...
        style={{ height: "calc(100vh - 30px)", scrollBehavior: "smooth" }}
        // id="chatContainer"
      >
          ...
          <div className="flex w-full items-center gap-4 px-3">
            ...
            <div className="flex w-full flex-col justify-center">
              <div className="flex h-7.5 items-center text-base font-bold text-dark-text-main">
                <EditableText
                  label={currentProject?.projectInfo?.projectName ?? currentProject?.name}
              ...
              <div className="flex h-7.5 w-full items-center text-sm text-dark-neutral-gray-700">
                <EditableText
                  className="w-full"
                  label={
                    currentProject?.projectInfo?.projectDescription ??
...
export default PageProjectData;
